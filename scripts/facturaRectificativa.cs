...
public class FacturaRectificativaScript : ITicketScript
{
    const int MW = 42;
    const int QW = 4;
    const int PW = 9;
    ...
    public byte[] Render(dynamic empresa, dynamic ticket, Encoding enc)
    {
        ...
        WordWrap(printer, "Factura Rectificativa: " + ticket.ticket);
        WordWrap(printer, "Rectifica Fac.: " + ticket.facturaOriginal);
        printer.Feed();

        // ── Datos cliente ─────────────────────────────────────────────────────
        printer.Text("-------- DATOS CLIENTE --------\n");
        printer.Feed();
        WordWrap(printer, "Nombre: " + cliente.razon_social);
        printer.Feed();
        WordWrap(printer, "Dirección: " + cliente.direccion + " " + cliente.cod_postal);
        printer.Feed();
        WordWrap(printer, "NIF: " + cliente.nif);
        printer.Feed(1);
        ...
        printer.Separator('=');
        printer.Text(Monto("TOTAL RECTIFICADO", total));
        printer.Feed(1);
