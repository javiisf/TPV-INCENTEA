...
public class AlbaranScript : ITicketScript
{
    const int MW = 42;
    const int QW = 4;
    const int PW = 9;
    ...
    public byte[] Render(dynamic empresa, dynamic ticket, Encoding enc)
    {
        ...
        printer.Text("ALBARÁN Nº: " + ticket.albaran + "\n");
        string fecha = OptStr(ticket, "fecha");
        if (fecha != "") printer.Text("Fecha:      " + fecha + "\n");
        printer.Feed();
        ...
        if (Has(ticket, "cliente"))
        {
            ...
            string razon = OptStr(cliente, "razonSocial");
            string nif   = OptStr(cliente, "nif");
            string dir   = OptStr(cliente, "direccion");
            string cp    = OptStr(cliente, "codPostal");
            if (razon != "") { WordWrap(printer, "Nombre: " + razon, MW); printer.Feed(); }
            if (nif   != "") { printer.Text("NIF:    " + nif   + "\n"); printer.Feed(); }
            if (dir   != "") WordWrap(printer, "Dir.:   " + dir, MW);
        ...
        // ── Pie ───────────────────────────────────────────────────────────────
        string textoLegal = OptStr(empresa, "textoLegal");
        if (textoLegal != "")
        {
            WordWrap(printer, textoLegal, MW);
            printer.Feed();
        }
        ...
        {
            printer.Feed(2);
            printer.Text("Recibí conforme:\n");
            printer.Feed(3);
            printer.Text("Firma: ___________________________\n");
            printer.Feed();
        }
